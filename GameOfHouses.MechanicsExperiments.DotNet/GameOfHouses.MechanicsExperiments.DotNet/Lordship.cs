...
namespace GameOfHouses.MechanicsExperiments.DotNet
{
    public class Lordship
    {
        ...
        public void Attack(Lordship target, Random rnd, bool getLiveInput = true, double acceptFealtyRatio = 0.5, double retreatRatio = 1)
        {
            ...
            {
                Console.WriteLine(attackingLordship.Lords.Last().FullNameAndAge + " cannot reach " + target.Name);
            }
            else
            {
                ...
                while (livingDefenders.Count() > 0 && livingAttackers.Count() > 0 && !endBattle)
                {
                    ...
                    else
                    {
                        ...
                        Console.WriteLine(attackingLordship.Lords.Last().FullNameAndAge + " has arrived at " + target.Name + " with an army of " + livingAttackers.Count());
                    }
                    ...
                    if (livingAttackers.Count() > livingDefenders.Count() * Constants.SURRENDER_RATIO && target.Lord.House.Lord.Location == target)
                    {
                        //SURRENDER 
                        Console.WriteLine("OFFER OF FEALTY: " + target.Lord.FullNameAndAge + "sues for peace and OFFERS FEALTY to " + attackingLordship.Lord.FullNameAndAge);
                        offerFealty = true;
                    }
                    ...
                    {
                        endBattle = true;
                        Console.WriteLine("RETREAT: " + attackingLordship.Lord.FullNameAndAge + " RETREATED from battle.");
                    }
                    ...
                    if (acceptFealty)
                    {
                        ...
                        Console.WriteLine("FEALTY: " + attackingLordship.Lord.House.Lord.FullNameAndAge + " accepted FEALTRY from " + target.Lord.House.Lord.FullNameAndAge);
                    }
                    if (!endBattle)
                    {
                        //proceed with attack
